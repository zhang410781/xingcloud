...
平台以“接入源”为管理边界接收 Alertmanager 和 Zabbix 告警。每个接入源拥有独立地址、独立 Bearer Token、业务上下文映射和运行开关，外部告警规则仍由原系统维护，平台不复制其规则。
...
- 同一个外部指纹在不同接入源中相互隔离。
- 业务归属依次采用映射规则、默认业务上下文；都不存在时进入“待绑定外部告警”。
...
| 默认业务上下文 | 映射规则均未命中时的归属，可留空 |
| 业务上下文映射 | 按标签或标准字段把告警分配给业务上下文 |
| 每分钟请求上限 | 单个接入源的请求限流，范围 1 到 10000 |
| 允许接入 | 关闭后拒绝该地址的所有请求 |
| 发送通知 | 控制首次、恢复、升级和研判结果是否允许外发 |
| 智能研判 | 控制新活动周期是否创建轻量研判任务 |

新接入源默认允许入库、开启研判、关闭通知。应先完成载荷测试和业务归属确认，再开启通知，避免与原告警链路重复发送。
...
映射规则按优先级从小到大执行，第一条完全命中的规则生效。一条规则内的多个条件是 AND 关系。

可匹配字段包括：

- 标准字段：`title`、`level`、`environment`、`cluster`、`namespace`、`service`、`resource`、`resource_type` 等。
- 原始标签：直接使用 `namespace`，或显式使用 `label.namespace`。
- 运算符：`==`、`!=`、`=~`、`!~`、`contains`。
...
namespace == kube-ai
cluster == production
目标业务上下文：生产 Kubernetes
```

配置页的“测试”功能可以粘贴一份 Alertmanager 或 Zabbix JSON，预览格式识别、级别、资源和最终业务归属，不会创建真实告警。

### 2.3 查看接入状态

接入源列表展示健康状态、通知/研判开关、活跃告警、待绑定告警、累计接收数和最近接收时间。
...
- 业务归属：当前业务上下文、全部业务上下文、待绑定外部告警。
- 告警详情：接入源、绑定状态、原始标签、通知记录、时间线和研判结果。

“待绑定”表示告警已成功入库，但接入源的映射规则和默认业务上下文都未得到有效归属。此时可先在“全部业务上下文”或“待绑定外部告警”中查看，再修正接入源映射；后续新告警和重复命中会采用新映射。
...
| 条件 | 示例 |
|---|---|
| 来源类型 | `source_type == alertmanager` |
| 接入源编码 | `ingress_source_code == production-alertmanager` |
| 接入源 ID | `ingress_source_id == 3` |
| 外部标签 | `label.namespace == kube-ai` |
| 标准维度 | `level == critical`、`cluster == production` |

推荐把 `ingress_source_code` 放入外部告警的聚合键，避免多个外部系统的同名告警被合并。
...
旧入口使用生产配置中的全局 `WEBHOOK_TOKEN` / `WEBHOOK_TOKENS`，没有接入源页面统计、独立 Token 或业务映射。已有调用方可以继续使用，新接入统一使用独立端点。
