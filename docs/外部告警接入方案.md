...
外部告警不创建平台 Prometheus 规则实例。是否发送由两层共同决定：

1. 接入源的“发送通知”开关。
2. 平台通知策略、静默、抑制、聚合和重复间隔。

配置路径：`可观测性 -> 告警规则 -> 告警源工作台 -> 通知策略`。

最小配置步骤：

1. 在告警源列表选择具体的 Alertmanager 或 Zabbix 来源。
2. 配置一名主负责人和可选协同负责人。
3. 新增通知策略，并为 Info、Warning、Critical 添加结构化路由。
4. 每条路由选择固定群、告警源负责人、接收组或资源负责人。
5. 按需设置标签匹配、聚合、静默、抑制和重复间隔。
6. 回到告警源概览，确认“发送通知”已开启。

例如 Warning 发送飞书群、Critical 发送应急群时，分别创建两条立即路由。需要 5 分钟未认领后打电话时，再增加一条 `Warning + 未认领 + 5 分钟 + 语音 + 告警源负责人` 路由，并把升级级别设为 Critical。语音渠道需要语音网关，负责人需要手机号和语音偏好。

每条策略强制属于一个告警源，不存在跨来源的全局兜底策略。策略默认将 `alert_source_code` 纳入聚合键，避免不同来源的同名告警混组。
...
统一 Zabbix 可以创建一个告警源。该来源配置公司级主负责人；产品级负责人通过资源 IP 匹配和“资源负责人”路由追加。
...
通知路由可以选择固定群、告警源负责人、指定接收组或资源负责人，最终接收范围按路由独立计算：
...
7. 在该告警源下新增分级通知路由，验证渠道和接收对象后，再开启告警源“发送通知”。
