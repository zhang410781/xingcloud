...
当前一级菜单为运行概览、AIOps、可观测性、任务中心、资源中心、工单系统、平台管理和系统管理。
...
| `resource_center` | 统一资源模型、稳定标识、负责人、关系、自动发现、运行时索引和告警匹配 |
| `cmdb` | 遗留接口兼容；默认关闭 Host/ConfigItem 双向同步，不再作为资产事实源 |
...
```mermaid
flowchart LR
    resource[资源中心稳定资源] --> context[可选业务上下文]
    prom[Prometheus] --> context
    log[单个日志源] --> context
    k8sapi[Kubernetes API] --> cluster[K8S 集群与节点资源]
    cluster --> context
