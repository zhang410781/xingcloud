...
class AlertSource(models.Model):
    ...
    converge_enabled = models.BooleanField('启用收敛', default=False)
    converge_group_fields = models.JSONField('收敛分组键', default=list, blank=True)
    converge_window_minutes = models.PositiveIntegerField('收敛窗口(分钟)', default=5)
    suppress_by_topology = models.BooleanField('拓扑抑制', default=False)
    suppress_ancestor_hops = models.PositiveIntegerField('抑制祖先层数', default=3)
...
class Alert(models.Model):
    ...
    converge_key = models.CharField('收敛聚合键', max_length=128, blank=True, default='', db_index=True)
    group_parent = models.ForeignKey(
        'self', on_delete=models.SET_NULL, null=True, blank=True,
        related_name='group_children', verbose_name='聚合父告警',
    )
    is_group_root = models.BooleanField('是否聚合根', default=False)
    ...
    suppressed_reason = models.CharField('抑制原因', max_length=255, blank=True, default='')
...
class AlertRule(models.Model):
    ...
    converge_enabled = models.BooleanField('启用收敛', default=False)
    group_fields = models.JSONField('收敛分组键', default=list, blank=True)
    suppress_by_topology = models.BooleanField('拓扑抑制', default=False)
    suppress_ancestor_hops = models.PositiveIntegerField('抑制祖先层数', default=3)
